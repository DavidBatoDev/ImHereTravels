...
/**
 * Keyboard navigation for filter/sort menus, which already carry the right roles
 * (`listbox` / `option` / `aria-selected`) but no key handling.
 *
...
export default function useListboxNav({
  ...
  // Held in a ref so an inline `() => setOpen(false)` doesn't re-run the keydown
  // effect on every render (which would yank focus back to the selected option
  // mid-nav). Synced in its own effect — never mutated during render.

...
import { contactLinks } from "@/data/contactUs";
      ...
      {contactLinks.items.map((item) => {
        ...
        return (
          <Link
            ...
            onClick={() => trackEvent("contact_click", { method: item.type })}
          ...
          >
            <span className="flex items-center gap-3">
              <Icon />
              <span className="font-body text-b2-mobile md:text-b2-desktop">{item.label}</span>
            </span>
            <ChevronRightIcon />
          </Link>
        );
